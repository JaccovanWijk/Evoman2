...
def offspring(solutions): #, old):
    ...
    if (min(top_fitness) < 0):
        positive = [x + min(top_fitness) for x in top_fitness]
        pop_weights = [x/sum(positive) for x in positive]
    else:
        pop_weights = [x/sum(top_fitness) for x in top_fitness]
    ...
    for c in range(len(population)): #new_children):
        ...
        parents = random.choices(top_pop, weights=pop_weights, k=2)
